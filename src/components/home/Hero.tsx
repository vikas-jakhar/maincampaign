                ...
                <div className="bg-white border xl:flex-row flex-col border-medium-gray rounded-[18px] flex justify-center xl:justify-between w-full py-4 px-5">
                    ...
                    <div className="flex items-center flex-col sm:flex-row justify-between">
                        ...
                        <div className="flex items-center my-2">
                            ...
                            {[...Array(2)].map((_, index) => (
                                <button id={index === 0 ? "setting" : "icon"} key={index} className="w-[50px] h-[50px] transition_slow hover:scale-95 ml-3.5 grid place-items-center border-light-gray text-off-gray bg-off-white border rounded-xl">
                                    <Icon iconName={index === 0 ? "graySettingIcon" : "bellIcon"} className='cursor-pointer relative z-10' />
                                </button>
                            ))}
                        </div>
                        <button id='profile' className="relative w-[57px] h-[57px] ml-7 pl-0.5 transition_slow hover:scale-95">
