...
const Card = () => {
    return (
        <div className='flex flex-row flex-wrap -mx-3.5 xl:mt-4'>
            {CARD_DATA.map((items, idx) => (
                <div key={idx} className="2xl:w-[20%] xl:w-4/12 sm:w-1/2 w-full px-3.5 mt-7 2xl:mt-0">
                    <div className="w-full h-full bg-white border relative overflow-hidden pb-16 border-medium-gray rounded-[18px] shadow-3xl pt-5 px-2 min-h-[159px]">
                        ...
                        <LineChart
                            lineColor={idx === 4 ? 'rgba(255, 105, 180, 1)' : 'rgba(75, 192, 192, 1)'}
                            fillColor={idx === 4 ? 'rgba(255, 182, 193, 0.3)' : 'rgba(144, 238, 144, 0.3)'}
                        />
                    </div>
                </div>
            ))}
